...
data <- read.csv("2024 Fall Batting Practice.csv")
...
for (batter in batters) {
  ...
  fancy_table_theme <- ttheme_minimal(
    core = list(fg_params = list(fontsize = 8, fontface = "plain"), bg_params = list(fill = "white", col = "black")),
    colhead = list(fg_params = list(fontsize = 8, fontface = "bold", col = "white"), bg_params = list(fill = "navyblue", col = "black"))
  )
  ...
  # Final Report Layout (Table → Zone Charts → Spray Chart)
